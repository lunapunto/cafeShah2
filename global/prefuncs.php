...
* Por default se añaden los siguientes estilos y fuentes:
  ...
  -Roboto Font
  -Materialize CSS
  -Confirm JS CSS (librería para alertas y confirmaciones)
  -global.css
...
* Por default se añaden los siguientes scripts:
  ...
  -Velocity JS
  -Confirm JS (librería para alertas y confirmaciones)
...
* @params $part (string) [default 'head'] - Determina en qué parte del HTML se hace el output del script. Default 'head'.
*
* (string) Devuelve el header evaluado
*/
function scriptsstyles($isadmin = false, $part = 'header'){
    ...
    $scripts['header']['jquery'] = 'https://ajax.googleapis.com/ajax/libs/jquery/3.1.1/jquery.min.js';
    $scripts['footer']['mat'] = $bower.'/materialize/bin/materialize.js';

    $scripts['footer']['velocity'] = $bower.'/velocity/velocity.min.js';

    $scripts['footer']['confirm'] = $bower.'/jquery-confirm2/dist/jquery-confirm.min.js';
    $scripts['footer']['confirmdefaults'] = js.'/jsconfirmdefaults.js';

    $styles['confirm'] = $bower.'/jquery-confirm2/dist/jquery-confirm.min.css';

    $styles['global'] = css.'/global.css?version='.currentVersion;

    if($isadmin){
      $styles['admin'] = css.'/admin.css?version='.currentVersion;
      $scripts['footer']['admin'] = js.'/admin.js?version='.currentVersion;
    }else{
      $styles['master'] = css.'/main.css?version='.currentVersion;
      $scripts['footer']['master'] = js.'/main.js?version='.currentVersion;
    }
    $o = '';
    /*
    * Estilos sólo en header
    */
    if($part == 'header'){
    ...
    foreach($scripts[$part] as $key=>$script){
